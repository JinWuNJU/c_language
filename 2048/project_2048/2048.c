...
int main() {
    ...
    TTF_Init();
    Window = SDL_CreateWindow("Game_2048", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 1000, SDL_WINDOW_SHOWN);
    ...
    while (SDL_WaitEvent(&MainEvent)) {
        ...
        switch (MainEvent.type) {
            ...
            case SDL_MOUSEBUTTONUP:
                ...
                if (MainEvent.button.x > 188 && MainEvent.button.x < 624 && MainEvent.button.y > 433 && MainEvent.button.y < 638) {
                    if (!IfBegin)
                        for (int i = 0; i < 2; ++i)
                            RandomCreate();
                    SDL_COMPILE_TIME_ASSERT(MainEvent, sizeof(SDL_Event) == sizeof(((SDL_Event *) NULL)->padding));
        ...
        while (SDL_PollEvent(&PlayEvent) || IfMsgBox) {
            switch (PlayEvent.type) {
                ...
                case SDL_MOUSEBUTTONUP:
                    ...
                    if (!IfMsgBox) {
                        if (dx < 20 && dy < 20 && UpButtonY > 200 && UpButtonY < 270) {
                          //  if (UpButtonX > 465 && UpButtonX < 533)
                           //     return 0;
                            if (UpButtonX > 325 && UpButtonX < 405)
                                Revoke();
                            else if (UpButtonX > 445 && UpButtonX < 525)
                                Restart();
                            else if (UpButtonX > 180 && UpButtonX < 255)
                                MsgBox(2);
                        ...
                        if (dx < 20 && dy < 20 && UpButtonY > 415 && UpButtonY < 665) {
                        /* if (UpButtonX > 261 && UpButtonX < 323) {
                                if (IfMsgBox == 2)
                                    Restart();
                                return 0;
                            }*/  //if (UpButtonX > 467 && UpButtonX < 548)
                                //Restart();
                            if (UpButtonX > 175 && UpButtonX < 620) {
                                if (IfMsgBox == 1) {
                                    IfMsgBox = 0;
                                    PlayStartTime += time(NULL) - PauseTime;
                                } else
...
void PrintScores() {
    ...
    SDL_Rect ScoreRect = {490 - ScoreSurface->w / 2, 100, ScoreSurface->w, ScoreSurface->h};
...
void StartAndLoad() {
    MainBackGroundSurface = IMG_Load("IMAGE/2048.png");
    PlayBackGroundSurface = IMG_Load("IMAGE/BB48F55DD83BF52523D07E302B80F918.png");
    for (int i = 0; i < 3; ++i) {
        char BoxName[20];
        sprintf(BoxName, "IMAGE/mggbox%d.png", i);
